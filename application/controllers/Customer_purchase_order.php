...
class Customer_purchase_order extends CI_Controller {
	...
	if($this->input->post('buyerid')){
		   ...
		   $query = $this->Admin_model->getdatafromtable('sellerpostproduct',$f); */
			...
			$data = array('sellerid' => $sellerid,'buyerid'=> $buyerid,'selprice'=>$selprice ,'sunits'=>$sunits,'punits'=>$punits,
			'productname' => $productname,'productid' => $productid, 'category' => $category,
			
			'pcities' => $pcities,'bquantity' => $bquantity, 'bunits'=> $bunits,
			'bprice' => $bprice,'bsupplyability'=>$bsupplyability,'uploadporder'=>$pic_array1,'price'=>$price,'units'=>$units,'quantity'=>$quantity);
